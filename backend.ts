...
async function resolveConnection(cfg: Config) {
...
async function generatePaths(args: {
  ...
  if (!config.useReasoning) request.reasoning = { source: 'off' }
  ...
  if (issues.length) throw new Error(`CYOA output failed validation: ${issues.join('; ')}`)
...
spindle.on('CHARACTER_MESSAGE_RENDERED', (payload: any) => {
