...
export function setup(ctx: Ctx) {
  ...
  function ensureCard(messageId: string, chatId?: string) {
    ...
    const guidanceInput = wrapper.querySelector('.pp-guidance-input') as HTMLTextAreaElement | null
    const guidanceSave = wrapper.querySelector('.pp-guidance-save-check') as HTMLInputElement | null
    const guidanceCancel = wrapper.querySelector('.pp-guidance-cancel') as HTMLButtonElement | null
    const guidanceGenerate = wrapper.querySelector('.pp-guidance-generate') as HTMLButtonElement | null

    regen?.addEventListener('click', () => {
      const data = dataByMessage.get(messageId)
      const resolvedChatId = data?.chatId || chatId
      if (!resolvedChatId) return
      if (guidancePanel) guidancePanel.hidden = true
    ...
    guide?.addEventListener('click', () => {
      ...
      if (!guidancePanel.hidden) window.setTimeout(() => guidanceInput?.focus(), 0)
    })
