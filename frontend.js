  ...
  `);
    ...
    function openWriterModal(chatId, messageId) {
        try {
            ...
            updateHistoryUi();
            prev.addEventListener('click', () => { if (writerHistoryIndex > 0) {
                writerHistoryIndex--;
                updateHistoryUi();
            } });
            next.addEventListener('click', () => { if (writerHistoryIndex < writerHistory.length - 1) {
                writerHistoryIndex++;
                updateHistoryUi();
            } });
  ...
  resize();
    ...
    function updateModelPickers() {
        ...
        const writerConn = cfg.writerConnectionId
            ? (conns.find((c) => String(c.id) === String(cfg.writerConnectionId)) || pathsConn)
            : pathsConn;
    ...
    function applyState(state) {
        ...
        const writerConnectionId = cfg.writerConnectionId && conns.some((c) => c.id === cfg.writerConnectionId)
        ...
        const chosenWriterConn = writerConnectionId === '__same_as_paths__'
            ? selectedPathsConn
            : conns.find((c) => String(c.id) === writerConnectionId);
