...
let openRouterCatalogCache = { fetchedAt: 0, models: [] };
...
function normalizeOpenRouterModels(rows) {
    ...
    for (const row of rows || []) {
        ...
        const outputModalities = Array.isArray(row?.architecture?.output_modalities)
            ? row.architecture.output_modalities.map((x) => String(x).toLowerCase())
            : [];
        ...
        const supported = Array.isArray(row?.supported_parameters)
            ? row.supported_parameters.map((x) => String(x).toLowerCase())
            : [];
...
function formatCortexEntity(entity) {
    ...
    const aliases = Array.isArray(entity.aliases)
        ? entity.aliases.map((x) => String(x || '').trim()).filter(Boolean).slice(0, 5)
        : [];
    ...
    const facts = factsRaw
        .map((x) => cleanRoleplayText(typeof x === 'string' ? x : (x?.content || x?.fact || x?.text || '')))
        .filter(Boolean)
        .slice(0, 8);
    ...
    try {
        ...
        if (targetIsOoc && allowOocOverride) {
            ...
            const priorNormal = originalTargetIndex >= 0
                ? messages.slice(0, originalTargetIndex).filter((m) => (m.role === 'user' || m.role === 'assistant') && !oocMessageIds.has(String(m.id || '')))
                : storyMessages;
        ...
        const recentUserTurns = throughTarget
            .filter((m) => m.role === 'user' && (!config.skipOoc || !oocMessageIds.has(String(m.id || ''))))
