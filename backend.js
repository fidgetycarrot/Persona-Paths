...
var OPENROUTER_MODEL_TTL_MS = 60 * 60 * 1000;
var openRouterCatalogCache = { fetchedAt: 0, models: [] };
function extractOpenRouterRows(result) {
...
function normalizeOpenRouterModels(rows) {
  const seen = new Set;
  const models = [];
  for (const row of rows || []) {
    ...
    if (!id || seen.has(id))
      continue;
    const outputModalities = Array.isArray(row?.architecture?.output_modalities) ? row.architecture.output_modalities.map((x) => String(x).toLowerCase()) : [];
    if (outputModalities.length && !outputModalities.includes("text"))
      continue;
    seen.add(id);
    const supported = Array.isArray(row?.supported_parameters) ? row.supported_parameters.map((x) => String(x).toLowerCase()) : [];
    ...
    models.push({
      ...
      reasoning: supported.includes("reasoning") || supported.some((x) => x.startsWith("reasoning."))
    });
  }
...
async function getOpenRouterModels(force = false) {
  ...
  if (!models.length)
    throw new Error("OpenRouter returned no usable text models. Manual model IDs remain available.");
...
`);
  ...
  const rejectedBlock = rejectedChoices.length ? rejectedChoices.slice(0, 8).map((choice, i) => `PATH #${i + 1} \u2014 ${choice.title || choice.intent || "Untitled"}:
...
function buildUserWriterSystemPrompt(cfg, hasDraft) {
  const requestedPov = cfg.pov === "auto" ? hasDraft ? "Preserve the POV/person used in the DRAFT. If the draft is ambiguous, infer it from recent USER turns." : "Infer POV/person from recent USER turns; default to first person if ambiguous." : `Keep the rewritten draft in ${cfg.pov} person unless the human deliberately wrote otherwise.`;
  const requestedTense = cfg.tense === "auto" ? hasDraft ? "Preserve the tense used in the DRAFT. If ambiguous, infer it from recent USER turns." : "Infer tense from recent USER turns; default to present tense if ambiguous." : `Keep the rewritten draft in ${cfg.tense} tense unless the human deliberately wrote otherwise.`;
  const task = hasDraft ? `Rewrite a HUMAN PLAYER'S partially edited role-play response into one smooth, paste-ready USER turn. The human may have selected one or more Persona Paths suggestions, manually changed them, added new ideas, or written the whole draft themselves. Their draft is the authority for WHAT they intend to do.` : `Write the HUMAN PLAYER'S next in-character role-play turn from scratch. Use the current scene, persona, recent portrayal, guidance, and story memory to produce a plausible user-side continuation. Do not write the assistant/NPC side of the scene.`;
...
`)}
` : ""}
${String(args.draft || "").trim() ? `DRAFT TO REWRITE \u2014 PRESERVE ITS INTENT AND CONTENT
${String(args.draft || "").trim()}

Rewrite that draft into one seamless USER turn. Preserve the human's choices and added ideas; fix prose, flow, duplicated seams, and voice only.` : `NO DRAFT WAS PROVIDED.
...
async function handleUserWriter(chatId, messageId, draft, direction = "", sourceIntents = [], userId) {
  const cleanDraft = cleanGeneratedChoiceText(draft);
  const cleanDirection = String(direction || "").trim();
  if (!chatId || !messageId)
    throw new Error("Persona Paths needs the current chat and assistant reply for User Writer.");
  ...
  const sceneMessages = targetIsOoc && !cleanSceneMessages.some((m) => String(m?.id || "") === messageId) ? [...cleanSceneMessages.slice(-Math.max(1, config.contextMessages - 1)), target] : cleanSceneMessages;
  ...
  if (!raw)
    throw new Error(`User Writer model returned no final content${response?.finish_reason ? ` (finish reason: ${response.finish_reason})` : ""}.`);
  const rewritten = cleanGeneratedChoiceText(stripFences(raw));
  if (!rewritten)
    throw new Error("User Writer model returned an empty response.");
  ...
  try {
    spindle.toast.success(cleanDraft ? "Draft polished." : "Draft written.");
  } catch {}
}
...
function personaSelectionKey(chatId, userId) {
...
async function resolvePersona(chatId, userId) {
  ...
  if (!persona)
    throw new Error("The selected persona is unavailable. Choose another persona or Use Lumiverse active persona.");
  return persona;
}
async function listPersonas(userId) {
  ...
  for (let offset = 0;; ) {
    ...
    personas.push(...data);
    offset += data.length;
    if (!data.length || offset >= page.total)
      break;
  }
...
async function sendState(userId) {
  ...
  try {
    ...
    personaListError = err?.message || String(err);
  }
  let personaError = "";
  try {
  ...
  try {
    ...
    if (payload.type === "set_chat_persona") {
      ...
      if (!chatId || activeChat?.id !== chatId)
        throw new Error("The chat changed. Refresh the persona selection.");
      ...
      if (personaId)
        config.chatPersonas[key] = personaId;
      else
        delete config.chatPersonas[key];
    ...
    if (payload.type === "rewrite_draft" || payload.type === "user_writer") {
      ...
      const sourceIntents = Array.isArray(payload.sourceIntents) ? payload.sourceIntents.map((x) => String(x)).filter(Boolean).slice(0, 6) : [];
      try {
        ...
        try {
          spindle.toast.error(message, { title: "User Writer" });
        } catch {}
    ...
    if (payload.type === "manual_rewrite_latest" || payload.type === "manual_user_writer_latest") {
      ...
      try {
        if (!spindle.permissions.has("chats"))
          throw new Error("The Chats permission is required to resolve the active chat for User Writer.");
        const activeChat = await spindle.chats.getActive(userId);
        if (!activeChat?.id)
          throw new Error("Open a Lumiverse chat before using User Writer.");
        ...
        if (!latestAssistant?.id)
          throw new Error("The active chat does not have an assistant reply to anchor User Writer.");
